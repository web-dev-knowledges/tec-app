    ...
    <div className={`grid col-start-2 col-span-${tableData.length} grid-cols-${tableData.length} gap-1`}>
      {
        tableData.map(column => {
          return (
            <div key={column.type}
                 className={`text-center font-bold p-2 ${column.isHighlighted ? 'bg-slate-400' : 'bg-slate-200'}`}>
              {column.type}
            </div>
          )
        })
  ...
  const renderTableCell = useCallback((record: any, config: TableRowConfig): ReactElement | null => {
    switch (config.type) {
      case TableRowType.Tick:
        return record[config.key] ? <CheckOutlined /> : null
      default:
    ...
    return (
      renderData.map(row => (
        <div key={row.key}
          ...
          <div className={'text-slate-800 font-semibold p-2 pl-0'}>{row.label}</div>
          {
            tableData.map((record: any) => (
              <div
                key={record.type}
                className={`flex items-center justify-center p-2 ${record.isHighlighted ? 'bg-slate-400' : 'bg-slate-200'}`}>
    ...
    <div className={`grid col-start-2 col-span-${tableData.length} grid-cols-${tableData.length} gap-1 mt-10`}>
      {
        tableData.map(record => (
          <div key={record.type} onClick={() => console.debug(record)}
               className={`uppercase bg-slate-800 text-white text-center p-2 cursor-pointer hover:opacity-75`}>
            select
          </div>
        ))
      }
      {
        tableData.map(record => (
          <div key={record.type} onClick={() => console.debug(record)}
               className={`uppercase text-slate-800 text-center p-2 cursor-pointer hover:opacity-75`}>
            see pricing
          </div>
        ))
      }
      ...
      {renderTableBody()}
      {
        !showFullContent && (
          <div onClick={showAllContent}
               className={`grid col-start-2 col-span-${tableData.length} bg-slate-300 cursor-pointer hover:opacity-75`}>
